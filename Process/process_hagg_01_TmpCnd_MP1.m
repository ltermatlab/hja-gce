...
source          = 'fetch_hagg_01_TmpCnd_MP1';
template        = 'LNDB_WS1_StreamCarbon';
sitecode        = 'STREAMCARBON_WS01';
profile         = 'LNDB_HJA_hagg_01_TmpCnd_MP1';
pn_dest_root    = localpath('pn_dest_root');
pn_dest_plot    = localpath('pn_plots_root');
html            = localpath('html');
email           = localpath('email');
pn_dest         = strcat(pn_dest_root,'MISC\STREAMCARBON');
pn_plots        = strcat(pn_dest_root,'MISC\STREAMCARBON');
c               = clock; % c = clock for real-time processing or yyyy for post processing
fn_dest         = sprintf('hagg_01_TmpCnd_MP1_10min_%d.mat', c(1));

%% call the harvester
%data_harvester_sql(source,template,sitecode,profile,pn_dest,pn_plots,html,email,fn_dest);
data_harvester_sql(source, ...
                    template, ...
                    sitecode, ...
                    profile, ...
                    pn_dest, ...
...
pn_xml         = strcat(pn_dest_root,'MISC\STREAMCARBON\dash\MP1');
