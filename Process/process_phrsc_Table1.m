...
source          = 'fetch_phrsc_Table1';
load 'C:\Users\kennedad\Dropbox\matlab\gce_datatools_380_gce_production\userdata\FINAL_AIRTEMP_QC4SIGMA_leap_ak.mat';    %added 20140903 to account for new qc stats file
qc_source       = data;
clear data
...
pn_dest         = strcat(pn_dest_root,'PHRSC');
pn_plots        = strcat(pn_dest_root,'PHRSC');
c               = clock; % c = clock for real-time processing or yyyy for post processing
fn_dest         = sprintf('phrsc_15min_%d.mat', c(1));
% fn_gap          = sprintf('phrsc_15min_%d_gap_times.txt', c(1));  % File name of text file listing gaps filled.
fn_gap          = 'Z:\HJA\Portal_dev\PHRSC\data\phrsc_15min_gap_times.txt';
reprocess       = 1;

%% call the harvester
%data_harvester_sql(source,template,sitecode,profile,pn_dest,pn_plots,html,email,fn_dest);
[msg,status] = phrsc_data_harvester_sql_v2(source, ...
    qc_source, ...
    template, ...
    sitecode, ...
    profile, ...
...
%This function fails if fn_dest does not exist.
%Add path names for harvest dashboard
pn_xml         = strcat(pn_dest_root,'PHRSC\dash\table1');
