...
sitecode        = 'RS04_91';
profile         = 'LNDB_HJA_RS04_91_5min';
% profile_hrly    = 'LNDB_HJA_RS04_91_hrly';
% profile_dly     = 'LNDB_HJA_RS04_91_dly';
pn_dest_root    = localpath('pn_dest_root');
pn_dest_plot    = localpath('pn_plots_root');
html            = localpath('html');
email           = localpath('email');
pn_dest         = strcat(pn_dest_root,'RS\RS04');
pn_plots        = strcat(pn_dest_root,'RS\RS04');
c               = 2015; % c = clock for real-time processing or yyyy for post processing
fn_dest         = sprintf('RS04_91_a_5min_%d.mat', c(1));
% fn_dest_hrly    = sprintf('RS04_91_a_hrly.mat');
% fn_dest_dly     = sprintf('RS04_91_a_dly.mat');
reprocess       =1;

%% call the harvester
%data_harvester_sql(source,template,sitecode,profile,pn_dest,pn_plots,html,email,fn_dest);
rs_data_harvester_sql_v2(source, ...
    qc_source, ...
    template, ...
    sitecode, ...
    profile, ...
...
pn_xml         = strcat(pn_dest_root,'RS\RS04\dash\91_a_105');
