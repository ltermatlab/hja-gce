function [s2,msg] = interp_missing(s,xcol,ycols,method,maxpts,logopt,flag,flagdef)
%Performs 1D interpolation to fill in missing values in a single data series using a specified method
%and flags the interpolated values. Note that the data set will be sorted by column 'xcol' automatically
%
%syntax: [s2,msg] = interp_missing(s,xcol,ycols,method,maxpts,logopt,flag,flagdef)
%
%inputs:
            ...
            for n = 1:length(ycols)
               ...
               if ~isempty(Inan)
                  ...
                  try
                     ...
                     y2 = y;  %init y2
                     y2(Ifirst:Ilast) = y_int;  %update y2 with interpolated values
                  catch
                     y2 = [];
                     ycols_error(n) = 1;
                  end
