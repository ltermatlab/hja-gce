function [s2,msg] = negative2zero(s,cols,metadata,flag)
%Converts negative values in the specified columns of a GCE Data Structure to zeros
%
%syntax: [s2,msg] = negative2zero(s,cols,metadata,flag)
%
%inputs:
      ...
      if ~isempty(cols)
         ...
         if ~isempty(Inonstring)
            ...
            s2.history = [s2.history ; ...
               {datestr(now)},{['converted negative values in column(s) ',cell2commas(s2.name(cols),1), ...
            ...
            if updates > 0
               ...
               if metadata == 1
                  ...
                  s2 = addmeta(s2,{'Data','Anomalies',anom},0,'negative2zero');
               end
