function msg = gce_insert(s,conn,tablename,cols,fieldnames,maxrows)
%Executes the MATLAB Database Toolbox 'insert' function for selected columns of a GCE Data Structure
%
%syntax:  msg = gce_insert(s,conn,tablename,cols,fieldnames,maxrows)
%
%input:
...
%
%notes:
%  1) the Database Toolbox 'insert' function is required
%  2) data column arrays will be inserted as formatted - use GCE Data Toolbox data format
%     and date conversion utilities before calling gce_insert if conversions are required
%  3) if maxrows > 0, insert.m will be called repeatedly until all records are inserted,
...
if nargin >= 3 && ~isempty(conn) && ~isempty(tablename) && gce_valid(s,'data')
   ...
   %check for Data Toolbox insert function
   if exist('database/insert','file') == 2
      ...
      if ~isempty(cols)
         ...
         if length(cols) == length(fieldnames)
            ...
            for n = 1:numpasses
               ...
               try
                  insert(conn,tablename,fieldnames,data);
               catch e
      ...
      msg = 'the function database/insert is not available in MATLAB';
   end
