function [msg,status] = data_harvester_sql(source,template,sitecode,profile,pn_dest,pn_plots,html,email,fn_dest)
...
if nargin >= 1
   ...
   if exist(source,'file') == 2
      ...
      if gce_valid(data)
         ...
         %
         %code for appending to existing data file or managing existing data files goes here
         %
         ...
         if ~isempty(data)
           ...
           %send the congratulatory email notification of what's been
           %processed
           if ~isempty(data)
