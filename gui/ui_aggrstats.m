function ui_aggrstats(op,s,h_cb,cb)
%GCE Data Toolbox aggregated statistics dialog
%
...
%validate op argument, check for passing in structure as first argument
if nargin == 0
   op = 'init';
elseif isstruct(op)
   s = op;
...
if strcmp(op,'init')  %build gui
   ...
   if length(findobj) > 1
      h_dlg = findobj('Tag','dlgAggrStats');
   else
      h_dlg = [];
   end
   ...
   if ~isempty(h_dlg)  %set focus to existing dialog
      ...
      prefs = [];
      if exist('ui_aggrstats.mat','file') == 2
         try
            vars = load('ui_aggrstats.mat','-mat');
         catch
            vars = struct('null','');
         end
         if isfield(vars,'prefs')
            prefs = vars.prefs;
            if ~isfield(prefs,'GroupText')
               prefs.GroupText = 0;
               prefs.CountText = 1;
               prefs.Delimiter = ';';
            end
      ...
      bgcolor = [.95 .95 .95];
      figpos = [max(1,0.5.*(res(3)-585)) max(50,0.5.*(res(4)-640)) 585 640];
      ...
      h_mnuLoad = uimenu('Parent',h_mnuFile, ...
         'Label','Load', ...
         'Callback','ui_aggrstats(''load'')', ...
         'Tag','mnuLoad', ...
         'UserData',pwd);

      h_mnuClose = uimenu('Parent',h_mnuFile, ...
         'Label','Close', ...
         'Callback','ui_aggrstats(''cancel'')', ...
         'Separator','on', ...
         'Tag','mnuCancel');

      uimenu('Parent',h_mnuEdit, ...
         'Label','Reset Lists', ...
         'Callback','ui_aggrstats(''newdata'')', ...
         'Tag','mnuReset');
      ...
      uicontrol('Parent',h_dlg, ...
         ...
         'ForegroundColor',[0 0 0], ...
         'Position',[270 445 305 162]);

      uicontrol('Parent',h_dlg, ...
         'Style','frame', ...
         'BackgroundColor',[.9 .9 .9], ...
         'ForegroundColor',[0 0 0], ...
         'Position',[270 130 305 305]);

      uicontrol('Parent',h_dlg, ...
         'Style','frame', ...
         'BackgroundColor',[.9 .9 .9], ...
         'ForegroundColor',[0 0 0], ...
         'Position',[8 50 568 75]);
      ...
      h_chkFlags = uicontrol('Parent',h_dlg, ...
         ...
         'Callback','ui_aggrstats(''flags'')');

      h_editFlags = uicontrol('Parent',h_dlg, ...
         'Position',[465 260 55 20], ...
         'Style','edit', ...
         'FontSize',10, ...
         'BackgroundColor',[1 1 1], ...
         'ForegroundColor',[0 0 0], ...
      ...
      h_cmdFlags = uicontrol('Parent',h_dlg, ...
         'Position',[526 258 40 22], ...
         'Style','pushbutton', ...
         'String','List', ...
         'Enable','off', ...
         'TooltipString','Choose flag codes from a list', ...
         'Callback','ui_aggrstats(''pickflags'')', ...
         'Tag','cmdFlags');
      ...
      h_editQC1 = uicontrol('Parent',h_dlg, ...
         ...
         'Callback','ui_aggrstats(''checknum'')', ...
         'Tag','editQC1');
      ...
      h_chkGroupText = uicontrol('Parent',h_dlg, ...
         'Position',[92 61 130 20], ...
         'Style','checkbox', ...
         'FontSize',9, ...
         'BackgroundColor',[0.9 0.9 0.9], ...
         'String','Group text values', ...
         'Value',prefs.GroupText, ...
         'Callback','ui_aggrstats(''grouptext'')', ...
         'TooltipString','Option to to combine text column values (i.e. concatenate) within each group');
      
      h_chkCountText = uicontrol('Parent',h_dlg, ...
         'Position',[222 61 150 20], ...
         'Style','checkbox', ...
         'FontSize',9, ...
         'BackgroundColor',[0.9 0.9 0.9], ...
         'String','Enumerate text values', ...
         'Value',prefs.CountText, ...
         'TooltipString','Option to enumerate the individual occurrances of each text column value when grouped');
      
      h_lblDelimiter = uicontrol('Parent',h_dlg, ...
         'Position',[375 60 150 18], ...
         'Style','text', ...
         'FontSize',10, ...
         'BackgroundColor',[.9 .9 .9], ...
         'String','Grouped Text Delimiter:');
      
      h_editDelimiter = uicontrol('Parent',h_dlg, ...
         'Position',[525 60 30 18], ...
         'Style','edit', ...
         'FontSize',9, ...
         'BackgroundColor',[1 1 1], ...
         'String',prefs.Delimiter, ...
         'TooltipString','String to use to separate multiple text column values when grouped');      
      ...
      uih = struct( ...
         ...
         'chkGroupText',h_chkGroupText, ...
         'chkCountText',h_chkCountText, ...
         'lblDelimiter',h_lblDelimiter, ...
         'editDelimiter',h_editDelimiter, ...
         'cmdEval',h_cmdEval, ...
      ...
      %set visibility of dependent group text fields
      ui_aggrstats('grouptext')

      %check for data - populate controls
      if ~isempty(s)
         ui_aggrstats('newdata')
      end
   ...
   if length(findobj) > 2
      h_dlg = gcf;
      if ~strcmp(get(h_dlg,'Tag'),'dlgAggrStats')
         h_dlg = [];
      end
   end
   ...
   if ~isempty(h_dlg)
      ...
      case 'newdata'  %init list indices for new data
      ...
      case 'update'  %update control states after change
      ...
      case 'eval'  %evaluate input to generate derived structure
         ...
         grouptext = get(uih.chkGroupText,'Value');
         textcount = get(uih.chkCountText,'Value');
         delim = get(uih.editDelimiter,'String');
         ...
         prefs = struct('StatOpt',statoptval, ...
            ...
            'GroupText',grouptext, ...
            'CountText',textcount, ...
            'Delimiter',delim);   %#ok<NASGU>
         
         %save dialog preferences
         pn = fileparts(which('ui_aggrstats.mat'));
         if isempty(pn)
            pn = [gce_homepath,filesep,'settings'];
         end
         if ~isdir(pn)
            pn = fileparts(which('ui_aggrstats'));
         end
         save([pn,filesep,'ui_aggrstats.mat'],'prefs')
         
         set(gcf,'Pointer','watch')
         drawnow

         %call appropriate function based on text group option
         if grouptext == 0
            [s2,msg] = aggr_stats(s,I_aggr,I_stat,statopt,flagopt,qcrules,tminmax);
         else
            [s2,msg] = aggr_stats_text(s,I_aggr,I_stat,statopt,flagopt,qcrules,tminmax,delim,textcount);
         end
      ...
      case 'load'  %load new data structure from disk

         %open file load dialog starting in last directory
         curpath = pwd;
         ...
         if fn ~= 0
            ...
            if err == 0 && isstruct(vars)
               ...
               if ~isempty(Isel)
                  ...
                  if gce_valid(s,'data')
                     %store new data structure, regenerate lists
                     uih.s = s;
                     set(h_dlg,'UserData',uih)
                     ui_aggrstats('newdata')
                  else
                     messagebox('init', ...
                        ['''',vnames{Isel},''' is not a valid GCE Data Structure'], ...
                        [], ...
                        'Error', ...
                        [.9 .9 .9]);
                  end
      ...
      case 'grouptext'  %handle dependent control states when group text option is selected
         
         %get group text option
         grouptext = get(uih.chkGroupText,'value');
         
         %determine dependent control state
         if grouptext == 1
            newstate = 'on';
         else
            newstate = 'off';
         ...
         set(uih.chkCountText,'Visible',newstate)
         set(uih.lblDelimiter,'Visible',newstate)
         set(uih.editDelimiter,'Visible',newstate)
