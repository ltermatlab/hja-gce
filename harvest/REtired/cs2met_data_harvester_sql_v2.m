function [msg,status] = cs2met_data_harvester_sql_v2(source,qc_source,template,sitecode,profile,pn_dest,pn_plots,html,email,fn_dest,reprocess)
...
if nargin >= 1
    ...
    if exist(source,'file') == 2
        ...
        if gce_valid(data)
            ...
            %structure
            col = name2col(data,{'AIR_150_AVG'}); % check for column to qc
            if ~isempty(col)
            ...
            if ~isempty(data)
                ...
                col = name2col(data,'PPT_N4_INST');
                %convert PPT_INST from in to mm
                if ~isempty(col)
                    [data,msg] = unit_convert(data,'PPT_N4_INST','mm','x*25.4');
                end
                ...
                if ~isempty(col)
                    [data,msg] = add_calcexpr(data, ...
                        '[NaN ; diff(PPT_N4_INST)]', ...            %calculate precip using 0.05mm/tip correction factor
                        'PPT_N4_DIFF', ...                       %new column name
                        'mm', ...                               %new column units
                        'Difference in precipitation', ...  %description
                        col+1, ...                              %position
                        0, ...                                  %replicate scalar values (no)
                        '', ...             %Q/C flag criteria for the new column 'x<0=''I'';x>5=''I'''
                ...
                if ~isempty(col)
                    [data,msg] = num_replace(data, ...
                        'PPT_N4_DIFF', ...           %columns to update
                        'PPT_N4_DIFF < 0.08', ...      %criteria to match
                ...
                col = name2col(data,'PPT_N4_AVG');
                %convert PPT_AVG from in to mm
                if ~isempty(col)
                    [data,msg] = unit_convert(data,'PPT_N4_AVG','mm','x*25.4');
                end
            ...
            if ~isempty(data)
                ...
                interpcols = {'ActTemp','PPT_N4_INST','PPT_N4_AVG'};
