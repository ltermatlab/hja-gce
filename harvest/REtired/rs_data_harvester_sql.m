function [msg,status] = rs_data_harvester_sql(source,qc_source,template,sitecode,profile,profile_hrly,profile_dly,pn_dest,pn_plots,html,email,fn_dest,fn_dest_hrly,fn_dest_dly,reprocess)
    ...
    if exist(source,'file') == 2
      ...
      if gce_valid(data)
         ...
         %begin elevated qc routine using seperate qc_stats file
            ...
            %structure
            col = name2col(data,{'AIRTEMP_MEAN_225_0_04','AIRTEMP_MEAN_225_0_02','AIRTEMP_MEAN_225_0_03','AIRTEMP_MEAN_225_0_01'}); % check for column to qc
            if ~isempty(col)
         ...
         interpcols = {'AIRTEMP_MEAN_225_0_04','AIRTEMP_MEAN_225_0_02','AIRTEMP_MEAN_225_0_03','AIRTEMP_MEAN_225_0_01'};
         ...
         if ~isempty(data)
           ...
           %Testing quick database inserts
           %check for target variables in reference stand for database inserts
%            col = name2col(data,{'Site','Date','AIRTEMP_MEAN_225_0_02'});
%            if ~isempty(col)
...
%                    conn, ...
%                    'met_test_table', ...
%                    {'Site','Date','AIRTEMP_MEAN_225_0_02','Flag_AIRTEMP_MEAN_225_0_02'}, ...
%                    [] ...
%                    );
