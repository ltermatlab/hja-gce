function [msg,status] = uplmet_data_harvester_sql(source,template,sitecode,profile,pn_dest,pn_plots,html,email,fn_dest,reprocess)
...
if nargin >= 1
   ...
   if exist(source,'file') == 2
      ...
      if gce_valid(data)
             ...
             col = name2col(data,'PPT_SA_INST');
                ...
                if ~isempty(col)
                    [data,msg] = num_replace(data, ...
                        ...
                        'PPT_SA_DIFF < 0.2', ...      %criteria to match
                ...
                if ~isempty(col)
                    [data,msg] = num_replace(data, ...
                        ...
                        'PPT_SH_DIFF < 0.2', ...      %criteria to match
                        0, ...                    %new value to assign
                        0, ...                      %change log option
                        '' ...                      %flag to assign for revised values ('' for none)
                        );
         ...
         interpcols = {'AIR_150_AVG','AIR_250_AVG','AIR_350_AVG','AIR_350A_AVG','AIR_450_AVG','BP_INST', ...
