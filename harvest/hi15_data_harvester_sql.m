function [msg,status] = hi15_data_harvester_sql(source,template,sitecode,profile,pn_dest,pn_plots,html,email,fn_dest,reprocess)
    ...
    if exist(source,'file') == 2
        ...
        if gce_valid(data)
            ...
            if ~isempty(data)
                ...
                %interpolate missing values when less than 5
                %master list of columns to interpolate
                interpcols = {'PRECIP_INST_455_0_01'};
                ...
                if ~isempty(col)
                    [data,msg] = unit_convert(data,'PRECIP_INST_455_0_01','mm','x*25.4');
                end
            ...
            if ~isempty(col)
                ...
                %        '[NaN ; diff(PRECIP_INST_455_0_01)]', ...           %calculate precip using 0.05mm/tip correction factor
                %        'PRECIP_DIFF_455_0_01', ...                         %new column name
                %        'mm', ...                                           %new column units
                ...
                %        'PRECIP_DIFF_455_0_01', ...           %columns to update
                %        'PRECIP_DIFF_455_0_01 < 0.25', ...      %criteria to match
                ...
                %check for BucketTips from snow lysimeter column
                pos = name2col(data,'TIPPING_B_TOT');
                
                %add calculated Precip column if BucketTips present
                if ~isempty(pos)
                    [data,msg] = add_calcexpr(data, ...
                        'TIPPING_B_TOT .* 25.4', ...                   %calculate precip using 0.05mm/tip correction factor
                        'LYS_TOT_0_0_01', ...                           %new column name
                        'mm', ...                                   %new column units
                        'Snow melt and precipitation totals from the tipping bucket lysimeter', ...  %description
                        pos+1, ...                                  %position
                        0, ...                                      %replicate scalar values (no)
                        'x<0=''I'';x>178=''Q''', ...                %Q/C flag criteria for the new column
                        'calculation');                             %variable type of the new column
                end
                
                %Remove uneeded columns
                col = name2col(data,'TIPPING_B_TOT');
                if ~isempty(col)
