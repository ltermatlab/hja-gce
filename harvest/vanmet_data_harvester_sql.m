function [msg,status] = vanmet_data_harvester_sql(source,qc_source,template,sitecode,profile,pn_dest,pn_plots,html,email,fn_dest,reprocess)
...
%      GCE Data Structure and error message a s output by running sql2gceds.m or equivalent)
    ...
    if exist(source,'file') == 2
        ...
        if gce_valid(data)
            ...
            if ~isempty(data)
                ...
                col = name2col(data,{'SOLAR_MEAN_850_0_01'});
                if ~isempty(col)
                    [data,msg] = num_replace(data, ...
                        {'SOLAR_MEAN_850_0_01'}, ...                        %columns to update
                        'SOLAR_MEAN_850_0_01 < 1', ...                      %criteria to match (Add when SR is < 1 and daylight = 1)
...
%                         'SW_IN_MEAN_600_0_02 < 1', ...                                            %criteria to match (Add when SR is < 1 and daylight = 1)
%                         0, ...                                                                    %new value to assign
%                         1, ...                                                                    %change log option
%                         '' ...                                                                    %flag to assign for revised values ('' for none)
%                         );
%                 end
                ...
                if ~isempty(col)
                    [data,msg] = num_replace(data, ...
                        ...
                        'RELHUM_MEAN_150_0_04','RELHUM_MAX_150_0_04','RELHUM_MIN_150_0_04'}, ...                   %columns to update
                ...
                if ~isempty(col)
                    [data,msg] = num_replace(data, ...
                        {'SNOWDEP_INST_0_0_01','SWE_INST_0_0_01','SNOWDEP_MED_0_0_01'}, ...                   %columns to update
