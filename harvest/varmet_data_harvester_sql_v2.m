function [msg,status] = varmet_data_harvester_sql_v2(source,qc_source,template,qc_template,sitecode,profile,pn_dest,pn_plots,html,email,fn_dest,reprocess)
    ...
    if exist(source,'file') == 2
        ...
        if gce_valid(data)
            ...
            if ~isempty(data)
                ...
                interpcols = {'AIRTEMP_MEAN_450_0_10','SNOWDEP_INST_0_0_05','SWE_INST_0_0_05','PRECIP_INST_455_0_02'};
            ...
            %structure
            col = name2col(data,{'AIRTEMP_MEAN_450_0_10'}); % check for column to qc
            if ~isempty(col)
            ...
            if ~isempty(data)
                ...
                % Additional QC
                col = name2col(data,'SNOWDEP_QUAL');
                if ~isempty(col)
                    % remove erroneous Quality values
                    [data,msg] = num_replace(data, ...
                        'SNOWDEP_QUAL', ...           %columns to update
                        'Date <= datenum(''8/11/2012 12:00:00'')', ...      %criteria to match
                ...
                col = name2col(data,'SNOWDEP_INST_0_0_05','SWE_MEAN_0_0_01');
                if ~isempty(col)
                    [data, msg] = apply_template(data, qc_template);
                end
            end
            
            if ~isempty(data)
                
                col = name2col(data,'SNOWDEP_INST_0_0_05');
                if ~isempty(col)
                    md = running_median(extract(data,'SNOWDEP_INST_0_0_05'),12,1,'back');
                    ...
                    [data,msg] = addcol( ...
                        ...
                        'SNOWDEP_MED_0_0_05', ...                            % the name of the new column (required)
                ...
                if ~isempty(col)
                    ...
                    if ~isempty(col)
                        ...
                        col = name2col(data,'PRECIP_INST_455_0_02');
                        if ~isempty(col)
                            % add new column for corrected precip diffs
                            [data,msg] = addcol( ...
                                data, ...                                        % original data structure (required)
                                Diffs_455, ...                                       % an array of calculated values (required)
                                'PRECIP_TOT_455_0_02', ...                       % the name of the new column (required)
                        ...
                        col      = {'PRECIP_TOT_455_0_02'};        %values to be flagged
            ...
            if ~isempty(data)
                ...
                if ~isempty(col)
                    [data,msg] = unit_convert(data,'SNOWDEP_INST_0_0_05','mm');
                end
                
                col = name2col(data,'SNOWDEP_MED_0_0_05');
                %convert SNOWDEP_MED from m to mm
                if ~isempty(col)
                    [data,msg] = unit_convert(data,'SNOWDEP_MED_0_0_05','mm');
                end
                
                col = name2col(data,'SWE_MEAN_0_0_05');
                %convert SWE_INST from half inches to inches
                if ~isempty(col)
                    [data,msg] = unit_convert(data,'SWE_MEAN_0_0_05','in','x*1.95'); %1.95 is the conversion coeff from field units to inches for the swe pillow
                end
                
                %convert SWE_INST from in to mm
                if ~isempty(col)
                    [data,msg] = unit_convert(data,'SWE_MEAN_0_0_05','mm');
                end
            end
...
%                 col = name2col(data,{'AIRTEMP_150_DIAG','AIRTEMP_250_DIAG','SNOW_DIAG'});
%                 if ~isempty(col)
%                     [data,msg] = deletecols(data,{'AIRTEMP_150_DIAG','AIRTEMP_250_DIAG','SNOW_DIAG'});
%                 end
            end
