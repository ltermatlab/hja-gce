function [msg,status] = varmet_data_harvester_sql_v3(source,qc_source,template,qc_template,sitecode,profile,pn_dest,pn_plots,html,email,fn_dest,reprocess)
...
%                 end
                ...
                col = name2col(data,'SNOWDEP_INST_0_0_05','SWE_INST_0_0_05');
                if ~isempty(col)
            ...
            if ~isempty(data)
                ...
                if ~isempty(col)
                    md = running_median(extract(data,'SNOWEP_INST_0_0_05'),12,1,'back');
            ...
            if ~isempty(data)
                ...
                % New sensor added to logger 302 and native output is in mm.
%                 col = name2col(data,'SWE_MEAN_0_0_05');
%                 %convert SWE_INST from half inches to inches
%                 if ~isempty(col)
%                     [data,msg] = unit_convert(data,'SWE_MEAN_0_0_05','in','x*1.92');
%                 end
                %convert SWE_INST from in to mm
                if ~isempty(col)
                    [data,msg] = unit_convert(data,'SWE_INST_0_0_05','mm');
                end
            end
