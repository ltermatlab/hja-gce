...
if nargin >= 2  %check for minimum arguments
   ...
   if cancel == 0  %datum OK, proceed
      ...
      %validate lat/lon
      if isnumeric(lat) && isnumeric(lon) && length(lat) == length(lon)
         ...
         Iinvalid = lon < -180 | lon > 180 | lat < -90 | lat > 90;
         Ivalid = find(~Iinvalid);  %get index of good coordinates
         lon(Iinvalid) = NaN;  %replate invalid lon with NaN
         lat(Iinvalid) = NaN;  %replace invalid lat with NaN
         
         %check for any valid coordinates to reproject
         if ~isempty(Ivalid)
