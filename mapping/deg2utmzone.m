function [zone,utm_east,utm_north,hem,errormsg] = deg2utmzone(lon,lat,datum,zone)
%Converts geographic coordinates from lat/lon degrees to UTM using the specified ellipsoid datum
%optionally specifying the UTM zone to return
%
...
%     [] = automatically determine from lon (default)
%
%outputs:
%  zone = vector of UTM zones (1-60)
   ...
   if cancel == 0  %datum OK, proceed
      ...
      if length(lat) == length(lon)  %check vector sizes
         ...
         if min(lon) >= -180 && max(lon) <= 180 && min(lat) >= -90 && max(lat) <= 90
            ...
            if length(zone) == 1
               zone = repmat(zone,length(lon),1);
            elseif length(zone) ~= length(lon)
               zone = [];
            end
            if isempty(zone)
