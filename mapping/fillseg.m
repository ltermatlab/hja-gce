...
%Fills polygon line segments separated by [nan nan] (eg. Coastline data)
%
%syntax: h = fillseg(X,c1,c2)
%
%input:
%  X = two column matrix with x and y values, where  line segments are separated by [NaN NaN]
%  c1 = color for polygon fill ([r g b])
%  c2 = edgecolor for polygon ([r g b])
%
%output:
%  h = array of polygon handles
%
%usage notes:
%  If c1 and c2 are three column matrices, the first row is used to
%  fill the first segment, the second row is used to fill the second segment, etc.
%
%by Rich Signell (rsignell@usgs.gov)
%
%code obtained from http://woodshole.er.usgs.gov/operations/sea-mat/

%Original function documentation from Rich Signell
%
