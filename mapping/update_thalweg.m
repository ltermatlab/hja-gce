function msg = update_thalweg(transect,thalweg)
%Updates Thalweg reference data for a named transect in 'thalweg_ref.mat' and 'thalweg_bnd.mat'
%
%syntax: msg = update_thalweg(transect,thalweg)
%
%input:
%   transect = transect name (used as a variable in 'thalweg_ref.mat' and 'thalweg_bnd.mat')
%   thalweg = 3-column array of longitude (degrees), latitude (degrees) and distance (km)
%      representing a Thalweg reference transect
%
%output:
%   msg = status message
%
%notes:
%   1) existing reference data for the specified transect will be updated, otherwise a new
%      transect will be added
%   2) if the function 'geo_simplify.m' is present, polygons will be simplified to 60 vertices, 
%      otherwise points will be skipped to simplify the polygon
%
%
...
if nargin == 2 && size(thalweg,2) == 3
   
   if exist('thalweg_ref.mat','file') == 2 && exist('thalweg_bnd.mat','file') == 2
      
      %load main reference database variables into structure
      v_ref = load('thalweg_ref.mat');
      
      %get variable names as cell array
      vars = fieldnames(v_ref);
      
      %match transect to variables (case insensitive)
      Itransect = find(strcmpi(transect,vars));
      
      %add/update variable
      if isempty(Itransect)
         v_ref.(transect) = thalweg;  %add as new variable
      else
         v_ref.(vars{Itransect}) = thalweg;  %update existing variable to preserve case
      end
      
      %get original file location
      fn = which('thalweg_ref.mat');
      
      %save variables to file
      save(fn,'-struct','v_ref')
      
      %load boundaries database      
      v_bnd = load('thalweg_bnd.mat');
      
      %get variable names
      vars = fieldnames(v_bnd);
      
      %calculate rough bounding polygon around Thalweg by padding and offsetting coordinates
      lon = thalweg(:,1);
      lat = thalweg(:,2);
      lon2 = [lon(1) + 0.02 ; lon + 0.02 ; lon(end) - 0.04 ; lon(end) - 0.04 ; flipud(lon) - 0.02 ; lon(1) + 0.02 ; lon(1) + 0.02];
      lat2 = [lat(1) + 0.03 ; lat + 0.03 ; lat(end) ; lat(end) ; flipud(lat) - 0.03 ; lat(1) - 0.03 ; lat(1) + 0.03];

      %simplify polygon to 60 vertices
      [lon0,lat0] = geo_simplify(lon2,lat2,60);
      
      %match transect to boundary variables
      Itransect = find(strcmpi(transect,vars));
      
      %add/update variable
      if isempty(Itransect)
         v_bnd.(transect) = [lon0(:) lat0(:)];  %add as new variable
      else
         v_bnd.(vars{Itransect}) = [lon0(:) lat0(:)];  %update existing variable to preserve case
      end
      ...
      msg = 'successfully updated reference transects';     
      
   else
      msg = 'the Thalweg reference files ''thalweg_ref.mat'' and/or ''thalweg_bnd.mat'' were not located';
   end
   
else
   msg = 'missing or invalid input arguments';
end
return


%Note: 'geo_simplify.m' was downloaded as public software from the "Matlab Snack Bar" at
%   http://woodshole.er.usgs.gov/staffpages/cdenham/public_html/snackbar/geo_simplify.m
%
%This code is included unmodified in the GCE Data Toolbox for convenience and may be removed in a future revision
