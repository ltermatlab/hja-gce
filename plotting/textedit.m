...
if nargin == 0
   ...
   %check for prior instance of dialog, close if found
   h_dlg = findobj('Tag','dlgTextEdit');
   if ~isempty(h_dlg)
      close(h_dlg)
   end

   %get handle of object clicked on
   h_cbo = gcbo;
   ...
   h_dlg = figure('Units','pixels', ...
      'Position',[max(0,0.5.*(res(3)-850)) max(50,0.5.*(res(4)-270)) 850 200], ...
      'Visible','off', ...
      'Color',[0.8 0.8 0.8], ...
