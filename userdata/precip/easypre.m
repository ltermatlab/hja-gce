...
function [Results] = easypre();

%function [Results] = easypre(Station, Probe_Code, GaugeCol);
%%% EASYPRE is a script to take precipitation data and
%%% run it through a state algorithm. It is based on the differences
%%% between raw measurements. It uses an accumulating array called RESULTS to 
%%% work line by line along an input file, generating a RAWOUT 5-minute output,
%%% a midnight-to-midnight (daily) output, and a formated MSO4313 output.
...
%%% Station can be UPLMET, PRIMET, VANMET, H15MET, CENMET, CS2MET. Gaugecol is the column of the gauge.
...
%%% Make a small look up for the columns and files:
    ...
    %%% import the concatenated data. This data was downloaded from PORTAL and combined by timestep.    
    %% For testing
    % dataset = 1;
        if dataset == 1;
            fprintf(1,'%s\n','now processing VANMET SA')
            fprintf(fid_log,'%s\n','vanmet-->')
            % d = importdata('filename.csv', ',',5)
            d = importdata('NEW_VAN_PRE.csv');
            GaugeCol = 8;
            FlagCol  = 9;
            Station = 'VANMET';
            Probe_Code = 'PPTVAN01';
            ...
            Method = 'PPT017';
            dMethod = 'PPT117';
            M2Mout = 'xMSO43_daily_VANMET_SA_2015.csv';
            HighResOut = 'xMS043_highres_VANMET_SH_2015.csv';
            fid1 = fopen(HighResOut,'w');

        elseif dataset == 2;
            fprintf(1,'%s\n','now processing H15MET');
            fprintf(fid_log,'%s\n','h15met-->')
            % d = importdata('filename.csv', ',',5)
            d = importdata('NEW_H15_PRE.csv');
            GaugeCol = 9;
            FlagCol = 10;
            Station = 'H15MET';
            Probe_Code = 'PPTH1501';
            ...
            Method = 'PPT013';
            dMethod = 'PPT113';
            M2Mout = 'xMS043_daily_H15MET_2015.csv';
            HighResOut = 'xMS043_highres_H15MET_2015.csv';
            fid1 = fopen(HighResOut,'w');

        elseif dataset == 3;
            fprintf(1,'%s\n', 'now processing UPLMET SA');
            fprintf(fid_log,'%s\n','uplmet stand alone-->')
            % d = importdata('filename.csv', ',',5)
            d = importdata('NEW_UPL_PRE.csv');
            Station = 'UPLMET';
            Probe_Code = 'PPTUPL01';
            GaugeCol = 11;
            FlagCol = 12;
            ...
            dMethod = 'PPt115';
            Height = 455;
            M2Mout = 'xMSO43_daily_UPLMET_SA_2015.csv';
            HighResOut = 'xMS043_highres_UPLMET_SA_2015.csv';
            fid1 = fopen(HighResOut,'w');

        elseif dataset == 4;
            fprintf(1,'%s\n','now processing UPLMET SH');
            fprintf(fid_log,'%s\n','uplmet sheltered-->')
            % d = importdata('filename.csv', ',',5)
            d = importdata('NEW_UPL_PRE.csv');
            Station = 'UPLMET';
            ...
            GaugeCol = 17;
            FlagCol = 18;
            M2Mout = 'xMSO43_daily_UPLMET_SH_2015.csv';
            HighResOut = 'xMS043_highres_UPLMET_SH_2015.csv';
            Method = 'PPT016';
            dMethod = 'PPT116';
            Height = 625;
            fid1 = fopen(HighResOut,'w');
        ...
        elseif dataset == 5;
            fprintf(1,'%s\n','now processing CENMET SA');
            fprintf(fid_log,'%s\n','cenmet stand alone-->')
            % d = importdata('filename.csv', ',',5)
            d = importdata('2014_CEN_PRE.csv');
            Station = 'CENMET';
            Probe_Code = 'PPTCEN01';
            GaugeCol = 10;
            FlagCol = 11;
            ...
            Method = 'PPT015';
            dMethod = 'PPT115';
            M2Mout = 'xMSO43_daily_CENMET_SA_2015.csv';
            HighResOut = 'xMS043_highres_CENMET_SA_2015.csv';
            fid1 = fopen(HighResOut,'w');

        elseif dataset == 6;
            fprintf(1,'%s\n','now processing CENMET SH');
            fprintf(fid_log,'%s\n','cenmet sheltered-->')
            % d = importdata('filename.csv', ',',5)
            d = importdata('2014_CEN_PRE.csv');
            Probe_Code = 'PPTCEN02';
            ...
            dMethod = 'PPT116';
            Height = 625;
            M2Mout = 'xMSO43_daily_CENMET_SH_2015.csv';
            HighResOut = 'xMS043_highres_CENMET_SH_2015.csv';
            fid1 = fopen(HighResOut,'w');

        elseif dataset == 7;
            fprintf(1,'%s\n','now processing CS2MET');
            fprintf(fid_log,'%s\n','cs2met-->')
            % d = importdata('filename.csv', ',',5)
            ...
            Method = 'PPT018';
            Height = 250;
            M2Mout = 'xMS043_daily_CS2MET_2015.csv';
            HighResOut = 'xMS043_highres_CS2MET_2015.csv';
            fid1 = fopen(HighResOut,'w');
         ...
         %   elseif Diff2 <= 0 & abs(Diff2)/10 < abs(Diff) + 0.1;
            ...
            FirstRecord = {DN, HumanDate, AccGauge, str2num(RawGauge), Baseline, Diff, CorrDiff, Flag, OriFlag}; 
            %%% The second record reflects these criteria:
            SecondRecord = {DN2, HumanDate2, str2num(AccGauge2), str2num(RawGauge2), Baseline2, Diff2, CorrDiff2, Flag2, OriFlag2};
            ...
            Baseline2 = Baseline; 
            %%% The second record reflects these criteria:
            SecondRecord = {DN2, cHumanDate2, AccGauge2, str2num(RawGauge2), Baseline2, Diff2, CorrDiff2, Flag2, OriFlag2};
            ...
            for i = 3:length(Results)
                ...
                if NotRaining == 0;
                    ...
                    elseif Diff3 < 0 & abs(Diff3)/10 >= abs(CompareDiff)+0.1
                        ...
                        Baseline3 = str2num(RawGauge3); % the baseline is reset
                    ...
                    elseif CompareRaw == NaN & str2num(RawGauge3) ~= NaN;
                        ...
                        Baseline3 = str2num(RawGauge3); % set the new baseline to something which is a real #
                    ...
                    else Baseline3 = CompareBase; Flag3 = 'M'; AccGauge3 = CompareAcc; RawGauge3 = num2str(CompareRaw); Rebounding = 0; CountM = CountM + 1;
                    ...
                    ThirdRecord = {DN3, cHumanDate3, AccGauge3, str2num(RawGauge3), Baseline3, Diff3, CorrDiff3, Flag3, OriFlag3};    
                ...
                elseif NotRaining == 1;
                    ...
                    elseif str2num(RawGauge3) == NaN | strcmpi(OriFlag3, '"M"') ==1;
            ...
            fprintf(fid1,'%s,%s,%s,%s,%d,%s,%s,%s,%4.2f,%s,%4.2f,%s,%s\n', 'MS043','13', Station, Method, Height,'1P', Probe_Code, Results{1,2}, 0, 'A', Results{1,4}, Results{1,9}, 'NA');
...
%             fclose(fid2)
         ...
         clearvars d Results HighResOut FirstRecord SecondRecord M2M M2Mout M2Mcount
         dataset = dataset + 1;
