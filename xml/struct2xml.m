...
%  wrapcolumn = word wrap column for long strings (default = 100, 0 for no wrapping)
...
if nargin >= 1
   ...
   if isstruct(s)
      ...
      %apply default left margin if omitted
      if exist('lmarg','var') ~= 1 || isempty(lmarg)
         lmarg = 0;
      end
      
      %apply default indent if omitted
      if exist('indent','var') ~= 1 || isempty(indent)
         indent = 3;
      end
      
      %apply default wrap if omitted
      if exist('wrap','var') ~= 1 || isempty(wrap)
         wrap = 100;
      elseif wrap > 0 && wrap < 30
         wrap = 30;  %set minimum wrap to prevent margin offset errors with long strings
      end
      
      %set default emptyfld if omitted
      if exist('emptyfld','var') ~= 1 || isempty(emptyfld)
         emptyfld = 0;
      end
